...
async function renderFiles(folder) {
    ...
    try {
        files = await fetch(`https://learning.rolandasseputis.lt:3344/Learning-Dashboard/json/files:${folder}`);
        if (files.ok) {
...
async function renderFileContent(fileName) {
    try {
        fileContent = await fetch(`https://learning.rolandasseputis.lt:3344/Learning-Dashboard/json/fileContent:${fileName}`);
        if (fileContent.ok) {
...
try {
    folders = await fetch('https://learning.rolandasseputis.lt:3344/Learning-Dashboard/json/folders');
    if (folders.ok) {
